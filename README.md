...
MYSQL_SSL=true
MYSQL_SSL_CA_FILE=./secrets/aiven-ca.pem
...
For Aiven, copy the Service URI from the dashboard. The backend supports `ssl-mode=REQUIRED`. Add the Aiven CA certificate as a Render Secret File named `aiven-ca.pem`, then set `MYSQL_SSL_CA_FILE=./secrets/aiven-ca.pem`. You can also paste the certificate directly into `MYSQL_SSL_CA`.
